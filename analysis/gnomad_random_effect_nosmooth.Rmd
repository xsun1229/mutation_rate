---
title: "Modelling the random effect without smoothing"
author: "XSun"
date: "2023-04-05"
...
y_max <- max(combine$SNV_exp_unsam, combine$SNV_pred_random)

fit1 <- lm(SNV_pred_random ~ SNV_obs_unsam  + 0, data = combine)
adj.rsq <- summary(fit1)$adj.r.squared
pval <- summary(fit1)$coefficients[,4]

random_plot <- ggplot(combine, aes(x = SNV_obs_unsam, y = SNV_pred_random)) +
  geom_point() +
  labs(title = "Random Effect", x = "Observed", y = "Predicted_random") +
...
grid.arrange(random_plot, roulette_plot, ncol = 2)
