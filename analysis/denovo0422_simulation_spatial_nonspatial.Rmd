...
1. Simulating the random effects(fold changes)

We simulated spatial fold changes as we did in [spatial-only model](https://xsun1229.github.io/mutation_rate/denovo0422_simulation_spatial.html) 
We simulated spatial fold changes as we did in [non-spatial-only model](https://xsun1229.github.io/mutation_rate/denovo0422_simulation_nonspatial.html)

Then we summed up the spatial and non-spatial fold changes, and normalized them, making $mean(\tilde\theta_i) =1$
...
plot(randeff,type = "l",ylab = "simulated fold change")
```
...
plot_model <- function(data, estimate_column, title, alpha_est = NULL) {
  ...
  p <- ggplot(data, aes(x = randeff_simulated, y = get(estimate_column))) +
    ...
    labs(x = "true fold change theta_i",
         y = paste("estimated fold change -", title),
         title = title) +
    theme_minimal()
...
If we estimate the fold change using non-spatial model first and then fit the spatial model, we lost the signal for non-spatial model 
